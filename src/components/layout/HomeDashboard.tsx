    ...
    <div className="flex flex-col h-full bg-transparent overflow-y-auto custom-scrollbar">
      ...
      >
        ...
        <motion.div variants={item} className="pt-2">
          ...
          <p className="text-[13px] text-foreground/55 font-medium mt-1">
        ...
        <motion.div variants={item} className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {stats.map((s) => (
            ...
            >
              ...
              <div>
                <p className="text-xl font-bold text-foreground group-hover:text-primary transition-colors">{s.value}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {recentConversations.map((ch) => (
              <button
                ...
                className="group flex flex-col gap-2 p-4 rounded-2xl bg-white/[0.02] border border-white/[0.04] hover:bg-white/[0.05] hover:border-primary/20 text-left transition-all duration-200 shadow-premium"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="flex items-center justify-center w-7 h-7 rounded-lg bg-white/[0.04] group-hover:bg-primary/10 transition-colors">
                      <Hash size={13} className="text-foreground/60 group-hover:text-primary transition-colors" />
                    </div>
                ...
                <div className="flex items-center justify-between mt-1">
                   <ArrowRight size={12} className="text-foreground/35 group-hover:text-primary/70 transition-colors" />
                </div>
              </button>
            ))}
        ...
        <motion.div variants={item}>
          <div className="flex items-center gap-2.5 mb-3">
            <div className="w-1 h-[10px] bg-primary/40 rounded-full shrink-0" />
            <span className="text-[10px] font-bold uppercase tracking-wider text-foreground/55">Recent Activity</span>
          </div>
          ...
          <div className="space-y-1">
            {activityFeed.map((act, i) => (
              <motion.div
                ...
                className="group flex items-start gap-4 p-3.5 rounded-2xl hover:bg-white/[0.03] transition-all cursor-pointer shadow-premium"
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline justify-between gap-1.5 min-w-0">
                    <span className="text-[12px] font-bold text-foreground/85 group-hover:text-foreground transition-colors truncate">{act.title}</span>
                    <span className="text-[10px] text-foreground/35 font-medium shrink-0 group-hover:text-foreground/55 transition-colors">
                      {formatDistanceToNow(act.timestamp, { addSuffix: true })}
                    </span>
                  </div>
                  <p className="text-[11px] text-foreground/55 font-medium mt-0.5 truncate leading-snug group-hover:text-foreground/75 transition-colors italic">
                    {act.description}
                  </p>
                </div>
              </motion.div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            {pendingIncoming.length > 0 && (
              ...
              >
                ...
                <div className="flex-1 text-left">
                  ...
                  <p className="text-[11px] text-foreground/65 font-medium mt-0.5">You have {pendingIncoming.length} people waiting to connect with you.</p>
                </div>
                <ArrowRight size={18} className="text-primary/75 group-hover:translate-x-1 transition-all" />
              </button>
            )}
            ...
            >
              ...
              <div className="flex-1 text-left min-w-0">
                <p className="text-[12px] font-bold text-foreground/85">Browse Channels</p>
                <p className="text-[10px] text-foreground/45 font-medium truncate">Discover public communities</p>
              </div>
            </button>
            ...
            >
              ...
              <div className="flex-1 text-left min-w-0">
                <p className="text-[12px] font-bold text-foreground/85">Find People</p>
                <p className="text-[10px] text-foreground/45 font-medium truncate">Search for friends and colleagues</p>
              </div>
            </button>
            ...
            >
              ...
              <div className="flex-1 text-left min-w-0">
                <p className="text-[12px] font-bold text-foreground/85">Create Channel</p>
                <p className="text-[10px] text-foreground/45 font-medium truncate">Start a new group conversation</p>
              </div>
            </button>
          </div>
        </motion.div>
